...
def Data_Backfill(line, line_name, place_and_text, ID, start_time, end_time, setMonth,Source_data):
    ...
    if progress != "已完成":
        ...
        Source_data.iloc[line, Source_data.columns.get_loc("工作地点和内容")] = str(Source_data.iloc[
                                                                                    line, Source_data.columns.get_loc(
                                                                                        "工作地点和内容")]) + "\n" + place_and_text
...
def main_4(setMonth):
    ...
    Source_data = pd.read_excel(Source_path, sheet_name="4、终端场避雷器试验", skiprows=[0])
    ...
    read_data = read_data[(read_data['工作地点'].str.contains('避雷器试验|避雷器带电试验')) | (
        read_data['工作内容'].str.contains('避雷器试验|避雷器带电试验'))]
    print(f"4、终端场避雷器试验已搜到关键字数据共有：{len(read_data)}")
    ...
    with pd.ExcelWriter(Source_path, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
        Source_data.to_excel(writer, sheet_name="4、终端场避雷器试验", startrow=1, startcol=0, index=False)
    return Source_data
