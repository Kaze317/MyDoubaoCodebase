            ...
            if voltage in "110kV": 
                ...
                if Planned_month >= 7: 
                    if 6 <= read_data_Month <= Planned_month:
                        ...
                        end_time = read_data_time
                        Data_Backfill(line, line_name, place_and_text, ID, start_time, end_time, setMonth,Source_data)
            elif voltage in "220kV": 
                for vo_start_month in range(1, 12, 3):
                    if read_data_Month in range(vo_start_month, vo_start_month + 3) and Planned_month in range(vo_start_month,
                                                                                                         vo_start_month + 3):
                        ID = read_data.iloc[i]["计划编号"]  
                        start_time = read_data.iloc[i]["实际开始时间"]
                        end_time = read_data_time
    ...
    for i in range(len(read_data)):
        ...
        if re.search(sRet, place_and_text):
            ...
            for start_month in range(1,12,2):
                if read_data_Month in range(start_month, start_month + 2) and Planned_month in range(start_month, start_month + 2):
                    ID = read_data.iloc[i]["计划编号"]  
                    start_time = read_data.iloc[i]["实际开始时间"]
                    end_time = read_data_time
                    Data_Backfill(line, line_name, place_and_text, ID, start_time, end_time, setMonth,Source_data)
...
def main_6(setMonth):
    ...
    Source_data = pd.read_excel(Source_path, sheet_name="6、电缆终端红外检测", skiprows=[0])
    ...
    read_data = read_data[(read_data['工作地点'].str.contains('终端红外')) | (
        read_data['工作内容'].str.contains('终端红外'))]
    print(f"6、电缆终端红外检测已搜到关键字数据共有：{len(read_data)}")
    ...
    with pd.ExcelWriter(Source_path, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
        Source_data.to_excel(writer, sheet_name="6、电缆终端红外检测", startrow=1, startcol=0, index=False)
    return Source_data
