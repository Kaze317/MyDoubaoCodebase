...
def main_8(setMonth):
    ...
    Source_data = pd.read_excel(Source_path, sheet_name="8、非直埋式中间接头红外检测 ", skiprows=[0])
    ...
    read_data = read_data[(read_data['工作地点'].str.contains('接头红外')) | (
        read_data['工作内容'].str.contains('接头红外'))]
    ...
    with pd.ExcelWriter(Source_path, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
        Source_data.to_excel(writer, sheet_name="8、非直埋式中间接头红外检测 ", startrow=1, startcol=0, index=False)
    return Source_data
