...
def dis_Time(data, text, idx, now_month, case_text, now_state, now_class, now_require):
    ...
    if (now_require == '1月1次' or now_require == '1月2次') and data.at[idx, '现存状态'] == '现存区段':
        if pd.isna(data.at[idx, f'{now_month}月最近一次完成时间']): 
            ...
            data.at[idx, '完成月份'] = f'{time_month}月已完成'
    elif now_require == '2月1次' and now_month % 2 == 0 and data.at[idx, '现存状态'] == '现存区段':
        if pd.isna(data.at[idx, f'{now_month}月最近一次完成时间']) and pd.isna(data.at[idx, f'{now_month - 1}月最近一次完成时间']):
            data.at[idx, '计划临期提醒'] = f'{now_month}月需要提醒'
        else: 
            data.at[idx, '完成月份'] = f'{time_month}月已完成'
    elif now_require == '3月1次' and now_month % 3 == 0 and data.at[idx, '现存状态'] == '现存区段':
        if pd.isna(data.at[idx, f'{now_month}月最近一次完成时间']) and pd.isna(data.at[idx, f'{now_month - 1}月最近一次完成时间']) and pd.isna(data.at[idx, f'{now_month - 2}月最近一次完成时间']):
            data.at[idx, '计划临期提醒'] = f'{now_month}月需要提醒'
        else: 
            data.at[idx, '完成月份'] = f'{time_month}月已完成'
    return data
...
def main():
    ...
    hongwai_data = cewen_data(output_file, all_data, '重要跨越区段测温', now_month)
    xunshi_data = xs_data(output_file, all_data, '重要跨越区段巡视', now_month)


    
    
    with pd.ExcelWriter(output_file, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
        hongwai_data.to_excel(writer, sheet_name="重要跨越区段测温", startrow=1, startcol=0, index=False)
    with pd.ExcelWriter(output_file, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
        xunshi_data.to_excel(writer, sheet_name="重要跨越区段巡视", startrow=1, startcol=0, index=False)
    
    return hongwai_data, xunshi_data
